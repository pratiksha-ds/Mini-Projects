...
select a.bidder_id, bid_status,(count(bid_status)/no_of_bids)*100 as percent 
from ipl_bidding_details a join ipl_bidder_points b 
...
select a.stadium_id,count(match_id),stadium_name,city 
from ipl_stadium a join ipl_match_schedule b 
on a.stadium_id = b.stadium_id 
group by stadium_id;
# 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?

select STADIUM_NAME,(sum(TOSS_WINNER=MATCH_WINNER)/count(a.MATCH_ID))*100 as match_won from ipl_match a,ipl_match_schedule b,ipl_stadium c 
where a.MATCH_ID=b.MATCH_ID and b.STADIUM_ID=c.STADIUM_ID  
group by b.STADIUM_ID 
order by STADIUM_NAME;

# 4.	Show the total bids along with bid team and team name.


select a.Bid_team,team_name,count(bid_team)
from IPL_BIDDING_DETAILS as a join IPL_team as b
where team_id=bid_team
group by bid_team;


# 5.	Show the team id who won the match as per the win details.

select win_details, match_winner from ipl_match group by win_details;
...
select player_name from ipl_team_players a join ipl_team b  
on a.team_id=b.team_id join ipl_player c on a.player_id=c.player_id 
where team_name='Mumbai Indians' and player_role='Bowler'and a.player_id=c.player_id;

# 8.	How many all-rounders are there in each team, Display the teams with more than 4 
# all-rounder in descending order.

select * from IPL_TEAM_PLAYERS;
select remarks, count(player_role),remarks
